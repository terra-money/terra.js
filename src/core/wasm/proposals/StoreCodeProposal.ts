...
> {
    ...
    public toAmino(legacy?: boolean): StoreCodeProposal.Amino {
        if (legacy) {
            throw new Error('Not supported for the network');
        }
    ...
    public toData(legacy?: boolean): StoreCodeProposal.Data {
        if (legacy) {
            throw new Error('Not supported for the network');
        }
        const { title, description, run_as, wasm_byte_code, instantiate_permission } = this;
    ...
    public toProto(legacy?: boolean): StoreCodeProposal.Proto {
        if (legacy) {
            throw new Error('Not supported for the network');
        }
        const { title, description, run_as, wasm_byte_code, instantiate_permission } = this;
    ...
    public packAny(legacy?: boolean): Any {
        ...
        return Any.fromPartial({
            ...
            value: StoreCodeProposal_pb.encode(this.toProto(legacy)).finish(),
        });
    }
    ...
    public static unpackAny(
        ...
        return StoreCodeProposal.fromProto(
            StoreCodeProposal_pb.decode(msgAny.value), legacy
        );
    }
}
