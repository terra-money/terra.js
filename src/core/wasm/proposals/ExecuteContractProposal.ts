...
> {
    ...
    public static fromAmino(data: ExecuteContractProposal.Amino, legacy?: boolean): ExecuteContractProposal {
        if (legacy) {
    ...
    public toAmino(legacy?: boolean): ExecuteContractProposal.Amino {
    ...
    public static fromProto(proto: ExecuteContractProposal.Proto, legacy?: boolean): ExecuteContractProposal {
        if (legacy) {
            throw new Error('Not supported for the network')
        }
        return new ExecuteContractProposal(
    ...
    public toProto(legacy?: boolean): ExecuteContractProposal.Proto {
        const { title, description, run_as, contract, execute_msg, coins } = this;
        if (legacy) {
            throw new Error('Not supported for the network')
        } else {
    ...
    public packAny(legacy?: boolean): Any {
        if (legacy) {
            ...
            return Any.fromPartial({
                ...
                value: ExecuteContractProposal_pb.encode(this.toProto(legacy)).finish(),
            });
        }
    }

    public static unpackAny(msgAny: Any, legacy?: boolean): ExecuteContractProposal {
    ...
    public static fromData(data: ExecuteContractProposal.Data, legacy?: boolean): ExecuteContractProposal {
        if (legacy) {
            throw new Error('Not supported for the network')
        }
        const { title, description, run_as, contract, msg, funds } = data as ExecuteContractProposal.Data;
        return new ExecuteContractProposal(
            title,
            description,
            run_as,
    ...
    public toData(legacy?: boolean): ExecuteContractProposal.Data {
        const { title, description, run_as, contract, execute_msg, coins } = this;
        if (legacy) {
            throw new Error('Not supported for the network')
        } else {
